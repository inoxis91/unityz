...
- **Side channels**: `lib/discord.ts` (discord.js bot; per-guild channel IDs live on the `guilds` row), `lib/cron.ts` (node-cron, Europe/Paris: daily event reminders, fee reminders on the 15th and last 5 days of the month), nodemailer for support (`routes/support.ts`), `services/wclService.ts` (Warcraft Logs v2 GraphQL, client credentials). Every integration no-ops with a warning when its env vars are missing. See `backend/.env.example`; Stripe also needs `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET`.
