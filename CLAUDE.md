...
- **Platform back-office (`/backoffice`, site owner only)**: `routes/platform.ts` → `services/platformGuildService.ts` (guild facts, list, details, admin actions) and `services/platformStatsService.ts` (overview, funnel, usage, reasons, health). These are the only cross-tenant queries: never import them from a guild route. Access is `requirePlatformAdmin` (`middlewares/platform.ts`): `users.bnet_id` in `PLATFORM_ADMIN_BNET_IDS`, 404 for everyone else, same-origin requests only, and a Battle.net login under 12 h (`session.authenticated_at`, else 401 `PLATFORM_REAUTH_REQUIRED`). Every action and guild view is written to `platform_audit_log`; internal notes live in `platform_guild_notes` (never on `guilds`, whose rows reach the browser). Admin actions (extend/grant access, reset trial, revoke) run under `FOR UPDATE`; a guild with a live Stripe subscription keeps its Stripe plan, and `applyInvoicePaid` uses `GREATEST` so a renewal never shortens a granted extension.
- **Subscription analytics**: `services/analytics.ts` `track()` writes `analytics_events` (fire-and-forget, never fails the request) at each step of the journey (login, guild discovery/selection, pricing page, checkout started/canceled/expired/completed, trial, payments, past due, cancel, end). The browser can only send `payment_viewed` / `checkout_canceled` (`routes/analytics.ts`, context from the session). `recordActivity` fills `user_activity_days` (DAU/WAU/MAU, cohorts), `payments` is the paid-invoice ledger (idempotent per invoice), `churn_feedback` holds survey answers (pricing page, trial end, mandatory cancel reason also sent to Stripe `cancellation_details`). A guild's funnel stage is its furthest step reached (`GUILD_FACTS_SQL`). Cron: owner digest DM at 9:00 and end-of-trial DM (with survey link) at 11:00, Europe/Paris; `lib/cronRuns.ts` tracks last runs for the Health tab. Add new events to the `AnalyticsEvent` union and to `bo.tl.*` i18n keys.
...
- Back-office (`components/backoffice`, route `/backoffice`, `platformGuard`, link in the profile menu when `is_platform_admin`): tab, filters and the open guild drawer live in the URL (`?tab=guilds&stage=3&guild=<id>`); tabs are `@defer`red and load with `rxResource` (`BackofficeService.version` reloads them after an action). Pure logic in `backoffice-utils.ts`. The non-conversion survey is `shared/feedback-survey` (pricing page, `/payment?feedback=trial_end`, cancellation in options).
